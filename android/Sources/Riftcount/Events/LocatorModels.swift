...
struct LocatorPage<Item: Decodable & Sendable>: Decodable, Sendable {
    let total: Int?
...
struct LocatorEvent: Decodable, Sendable, Identifiable {
    let id: Int
    let name: String
...
struct LocatorEventSettings: Decodable, Sendable {
...
struct LocatorPhase: Decodable, Sendable, Identifiable {
    let id: Int
...
struct LocatorRound: Decodable, Sendable, Identifiable {
    let id: Int
    let roundNumber: Int
    let status: String?
...
struct EventRoute: Hashable, Sendable {
    let id: Int
    let alias: String?
}

/// Pushes the store search screen.
struct StoreSearchRoute: Hashable, Sendable {}

/// Pushes a store's detail screen (game-store UUID).
struct StoreRoute: Hashable, Sendable {
    let id: String
}

/// Pushes the favorite-stores events calendar.
struct StoreCalendarRoute: Hashable, Sendable {}

// MARK: - My events (registrations)

struct LocatorUserEventStatus: Decodable, Sendable, Identifiable {
    let id: Int
...
func isActiveRegistration(_ status: String?) -> Bool {
    guard let status else { return false }
    return ["COMPLETE", "CHECKED_IN"].contains(status.uppercased())
}

struct LocatorEventSummary: Decodable, Sendable, Identifiable {
    let id: Int
    let name: String
    let startDatetime: Date?
    let endDatetime: Date?
...
struct LocatorMatch: Decodable, Sendable, Identifiable {
    let tableNumber: Int?
    let podNumber: Int?
    let status: String?
    let matchIsBye: Bool?
...
struct LocatorMatchPlayer: Decodable, Sendable {
...
struct LocatorMyMatch: Decodable, Sendable {
    let id: Int
    let tableNumber: Int?
    let status: String?
...
struct LocatorMatchRelationship: Decodable, Sendable, Identifiable {
...
struct LocatorMatchUserStatus: Decodable, Sendable {
    let id: Int
    let bestIdentifier: String?
    let matchesWon: Int?
    let matchesLost: Int?
...
struct LocatorMatchUser: Decodable, Sendable {
    let id: Int
    let bestIdentifier: String?
...
struct ResolvedMyMatch: Sendable, Identifiable {
    let matchID: Int
    let tableNumber: Int?
    let status: String?
...
struct LocatorStanding: Decodable, Sendable, Identifiable {
    let rank: Int
    let tvDisplayName: String
    let matchesWon: Int?
    let matchesLost: Int?
...
struct LocatorRosterEntry: Decodable, Sendable, Identifiable {
    let tvDisplayName: String
    let registrationStatus: String?
    let checkedIn: Bool?
    let profileImageUrl: String?
...
struct LocatorDeckSubmissions: Decodable, Sendable {
    let totalSubmissions: Int?
    let submissions: [LocatorDeckSubmission]
}

struct LocatorDeckSubmission: Decodable, Sendable, Identifiable {
    let id: String
    let deckId: String?
    let deckName: String?
    let bestIdentifier: String?
...
struct LocatorStoreWrapper: Decodable, Sendable, Identifiable {
    let id: String
    let store: LocatorStore
}

struct LocatorStore: Decodable, Sendable, Identifiable {
    let id: Int
    let name: String
    let latitude: Double?
...
struct LocatorStoreEvent: Decodable, Sendable, Identifiable {
    let id: Int
    let name: String
    let startDatetime: Date?
    let displayStatus: String?
