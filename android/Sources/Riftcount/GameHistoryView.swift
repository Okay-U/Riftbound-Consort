...
/// Game history, ported from iOS: summary (record, win rate, avg duration,
/// turn-order splits) + per-game rows with delete.
/// Deferred: GameReviewView drill-down, edit sheet.
enum GameHistoryScope: Hashable {
    case all
    case deck(UUID?)
}

/// Navigation payload for history pushes.
struct GameHistoryRoute: Hashable {
