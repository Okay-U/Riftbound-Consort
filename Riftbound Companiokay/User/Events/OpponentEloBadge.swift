//
//  OpponentEloBadge.swift
//  Riftbound Companiokay
//
//  Scouting row for the "Your match" card: both sides' eloshowdown tier crest
//  and current ELO, mirrored under the two player names, plus the head-to-head
//  line from the signed-in player's perspective. Players resolve from their
//  Riftbound id (display name as fallback); a side with no eloshowdown profile
//  is simply left out, and the whole row hides when neither side has data.
//

import SwiftUI
import os

private let badgeLog = Logger(subsystem: "com.pitopia.riftcount", category: "EloBadge")
...
struct OpponentEloBadge: View {
    ...
    private func load() async {
        ...
        async let opponentPlayer = resolve(id: riftboundID, name: opponentName, label: "opponent")
        async let mePlayer = resolve(id: myRiftboundID, name: myName, label: "me")
        let (opponent, me) = await (opponentPlayer, mePlayer)

        guard opponent != nil || me != nil else {
            badgeLog.info("[EloBadge] neither side resolved — hiding")
            phase = .hidden
            return
        }
        ...
        if opp?.isEmpty != false, mine?.isEmpty != false, h2h?.hasHistory != true {
            badgeLog.info("[EloBadge] resolved but empty — hiding")
            phase = .hidden
            return
        }
    ...
    private func resolve(id: Int?, name: String?, label: String) async -> EloPlayer? {
        ...
        guard let name = name?.trimmingCharacters(in: .whitespaces), name.count >= 3,
            ...
            badgeLog.info("[EloBadge] \(label, privacy: .public) unresolved (rid=\(id.map(String.init) ?? "nil", privacy: .public) name=\(name ?? "nil", privacy: .public))")
            return nil
        }
        return player
    }
