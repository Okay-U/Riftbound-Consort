...
struct EventDetailView: View {
    ...
    @State var standingsInFlight = false
    @State var playerQuery = ""
    @State var deckCards: [String: String] = [:]
    @State var deckCount = 0
    ...
    @State var myResults: [RoundResult]?
    @State var myResultsLoading = false
    ...
    private func roundClock(_ ends: Date) -> some View {
        SecondTicker { now in
            let left = ends.timeIntervalSince(now)
            HStack(spacing: 6) {
                ClockGlyph(color: left > 0 ? EventsTheme.green : EventsTheme.gold)
            ...
            .background(Capsule().fill(left > 0 ? EventsTheme.greenSoft : EventsTheme.gold.opacity(0.14)))
            .frame(maxWidth: .infinity, alignment: .leading)
        }
    }
    ...
    private func upcomingCard(_ event: LocatorEvent, _ capacity: LocatorEventCapacity?) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let start = event.startDatetime {
                SecondTicker { now in
                    Text(Self.countdownText(to: start, now: now))
    ...
    @ViewBuilder
    func playerSearchSection(_ data: Loaded) -> some View {
        ...
        if data.standings.count + data.matches.count + roster.count > 4 {
            VStack(alignment: .leading, spacing: 11) {
                EventsSectionHeader("Find a player") {
    ...
    func searchablePlayers(_ data: Loaded) -> [FoundPlayer] {
        let cut = data.event.resolvedCutSize
    ...
    func foundPlayerRow(_ hit: FoundPlayer) -> some View {
        HStack(spacing: 12) {
    ...
    private func rosterSection() -> some View {
        if !roster.isEmpty {
            ...
            VStack(alignment: .leading, spacing: 11) {
                EventsSectionHeader("Who's playing (\(active.count))") {
    ...
    private func roundChip(_ round: LocatorRound, isSelected: Bool, isCurrent: Bool, event: LocatorEvent) -> some View {
        ...
        .background(Capsule().fill(isSelected ? EventsTheme.green : Color.white.opacity(0.06)))
        .onTapGesture { selectRound(round, currentID: event.currentRound?.id) }
    }
    ...
    private func myResultsCard(_ data: Loaded) -> some View {
        ...
        if data.myName != nil, !completed.isEmpty, !data.event.isUpcoming {
            VStack(alignment: .leading, spacing: 11) {
                EventsSectionHeader("Your results") {
                    ClockGlyph(color: EventsTheme.green)
                }
...
/// Drawn clock face (clock/timer symbols are not in SkipUI's map).
struct ClockGlyph: View {
    var size: CGFloat = 12
    var color: Color = EventsTheme.green

    var body: some View {
        ZStack {
            Circle().stroke(color, lineWidth: 1.5)
            Capsule().fill(color)
                .frame(width: 1.5, height: size * 0.32)
                .offset(y: -size * 0.16)
            Capsule().fill(color)
                .frame(width: size * 0.26, height: 1.5)
                .offset(x: size * 0.13)
        }
