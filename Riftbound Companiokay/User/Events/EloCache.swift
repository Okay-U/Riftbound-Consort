...
//
//  Caching layer in front of EloShowdownAPI. The Profile segment is torn down
//  and rebuilt on every segment switch, which used to refire ~9 requests per
//  visit against a 60/hour anonymous rate limit. This actor memoizes responses
//  with a TTL so re-entering the Profile (or re-showing an opponent badge)
//  inside the window costs zero requests.
//
