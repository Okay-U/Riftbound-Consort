...
enum DeckTextFormat {
    ...
    private static func normaliseName(_ s: String) -> String {
        var t = s.lowercased()
        // Strip parenthesised qualifiers like "(Showcase)" — manual scan
        // instead of regex (portable across the Android SDK).
        stripParens(&t)
    ...
    /// Removes all "(...)" spans, including a leading space if present.
    private static func stripParens(_ t: inout String) {
        while let open = t.firstIndex(of: "("),
              let close = t[open...].firstIndex(of: ")") {
            var start = open
            if start > t.startIndex {
                let prev = t.index(before: start)
                if t[prev] == " " { start = prev }
            }
            t.removeSubrange(start...close)
        }
    }
}
