...
/// Read-only client for the eloshowdown.com public API (api/v1), ported from
/// iOS. Keyless: anonymous access is rate-limited per IP (60 req/hour), which
/// is plenty for a profile screen and avoids embedding a shared secret in the
/// app binary. HTTPS only. Attribution is shown in the UI.
...
final class EloShowdownAPI: EloShowdownService {
    ...
    init() {
        ...
        // Date.ISO8601FormatStyle (FoundationEssentials, available on Android;
        // ISO8601DateFormatter is not guaranteed there).
